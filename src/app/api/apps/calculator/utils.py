...
def analyze_image(img: Image, dict_of_vars: dict):
    ...
    prompt = (
        ...
        f"1. Simple mathematical expressions like 2 + 2, 3 * 4, 5 / 6, 7 - 8, etc.: In this case, solve and return the answer in the format of a LIST OF ONE DICT [{{'expression': given expression, 'result': calculated answer}}]. "
        ...
        f"3. Assigning values to variables like x = 4, y = 5, z = 6, etc.: In this case, assign values to variables and return another key in the dict called {{'assign': True}}, keeping the variable as 'expression' and the value as 'result' in the original dictionary. RETURN AS A LIST OF DICTS. "
        ...
        f"5. Detecting Abstract Concepts that a drawing might show, such as love, hate, jealousy, patriotism, or a historic reference to war, invention, discovery, quote, etc. USE THE SAME FORMAT AS OTHERS TO RETURN THE ANSWER, where 'expression' will be the explanation of the drawing, and 'result' will be the abstract concept. "
        ...
        f"DO NOT USE ```json OR ```python OR ANY OTHER MARKDOWN FORMATTING. "
        f"MAKE SURE TO DELETE ANY ```json OR ```python OR ANY OTHER MARKDOWN FORMATTING. "
        f"PROPERLY QUOTE THE KEYS AND VALUES IN THE DICTIONARY FOR EASIER PARSING WITH Python's ast.literal_eval."
    )
    response = model.generate_content([prompt, img])
    print("Raw response:", response.text)
    response_text = response.text.replace("`", "").replace("json", "")
    print("Formatted raw response:", response_text)
